...
- **Domain construction and mutation placement.** If the aggregate doesn't already have a factory/mutator suited to this, don't bolt a legacy-shaped `Create`/`Update` directly onto the aggregate if the aggregate is reasonably expected to grow a "real" first-class version of that operation later — instead add a C# 14 extension member scoped to `Neba.Api.Legacy` (e.g. `{Aggregate}.CreateFromLegacy`, `{Aggregate}.ApplyLegacyUpdate`), so the legacy-shaped mapping/validation logic is deleted with the rest of the backdoor at sunset instead of permanently occupying the aggregate's real API surface. This applies the same way to updates as it does to creates (confirmed on the `UpdateBowler` action) — `{Aggregate}.ApplyLegacyUpdate(...)` is an extension member, not a method on the aggregate itself, matching `CreateFromLegacy`.
  - **Mechanical wrinkle for updates specifically**: an extension member cannot assign an existing instance's `init`-only properties (`init` accessors only run during construction/`with`, and plain classes — unlike records — don't support `with` at all). If the properties an update needs to touch are `{ get; init; }`, they need to become `{ get; internal set; }` (or `private set` if only the aggregate's own assembly ever needs it — `internal` matches the existing "same-assembly, not exposed further" convention from child-entity factories) as a real, permanent change to the aggregate itself. This setter-visibility change is *not* legacy-shaped and is fine to keep post-sunset — it's exactly what the aggregate's own eventual first-class `Update` method will need too, so it isn't wasted once the extension member is deleted. Only the extension member's own mapping/validation logic goes away at sunset, not the setters it relies on.
  - If the aggregate already has a durable method that does exactly what's needed, reuse it — don't duplicate logic just to keep it inside `Legacy/`.
...
- **Cache eviction.** The sync job writes through `AppDbContext` directly — it bypasses the command-handler pipeline that normally calls `IFusionCache.RemoveByTagAsync(...)` after a save, so any cached query whose data this job touches will go stale unless the job evicts it itself. For the write(s) this action performs, check `src/Neba.Api/Caching/CacheDescriptors.cs` for every existing tag whose underlying query reads the same data (don't assume only the "obvious" tag — e.g. a bowler-name update can affect a champions list that joins in bowler names, not just a per-bowler tag) and list each tag the job must evict, right after `SaveChangesAsync`, matching `GenerateSeasonStatsJob`'s existing pattern (`Legacy/Tournaments/Stats/GenerateSeasonStatsJob.cs`, the only current legacy job that already does this). If no existing cached query reads the written data, say so explicitly rather than silently omitting the section — see `docs/plans/legacy-jobs-cache-invalidation-review.md` for the worked example of this analysis across all existing legacy jobs.
- **DI registration of the new `IValidator<TRequest>` — required in three places, all three named explicitly in the plan, not left implicit:**
  1. **Production**: `builder.Services.AddScoped<IValidator<{Action}Request>, {Action}RequestValidator>();` in `LegacyConfiguration.cs`'s `AddLegacy()`, alongside the existing sibling registrations.
  2. **This action's own test file**: the new `{Action}EndpointTests.InitializeAsync()` registers its own request's validator, plus **every other validator already registered in the other `/legacy` test files' `InitializeAsync()`** (not just its own) — `_app.MapLegacyGroup()` maps and builds route metadata for *every* endpoint in the `/legacy` group on first request, so a missing sibling `IValidator<T>` throws `InvalidOperationException: Failure to infer one or more parameters` for the whole group, not just the sibling's own tests.
  3. **Every existing `/legacy` endpoint test file's `InitializeAsync()`** must be updated in the same change to also register the *new* action's `IValidator<{Action}Request>` — grep `tests/Neba.Api.Tests/Legacy/**/*Tests.cs` for `MapLegacyGroup()` to enumerate every file needing the addition, and list each file by name in the plan so the implementer doesn't miss one. (This exact gap — a new action's validator added to production and its own tests but not backfilled into prior sibling test files — broke 8 tests across two files after `NewTournament` shipped; don't repeat it.)
  - Add a one-line code comment at each test registration site (matching the existing convention already in these files) explaining *why* sibling validators are required, not just that they are.
