...
- **Always these stages, gated on confirmation**, in order:
  1. Permission/role determination
  2. UI flow sketch (lightweight) — skipped only if the feature has no UI surface
  3. Phase 1 functional draft
  4. Phase 1 code draft
  5. Phase 2 functional draft
  6. Phase 2 UI mockups (HTML) — skipped only if Step 8 concluded there is genuinely no UI surface
  7. Phase 2 code draft
...
## Step 3 — Permission/role determination
...
Record the decision (policy name — new or existing; role(s) it's granted to; whether a new role was introduced and why) so it can be captured under "Decisions locked in during scoping" in Step 5.

Do not proceed until the user has confirmed the permission/role decision.

## Step 4 — UI flow sketch (lightweight)
...
## Step 5 — Initialize the plan file
...
If Step 1/Step 2/Step 3/Step 4 surfaced decisions, assumptions, or things the user explicitly ruled in/out while scoping — including the permission/role decision from Step 3 and the confirmed flow from Step 4 — capture them under a short `## Decisions locked in during scoping` section between the title and `## Phase 1` — this is where cross-phase context that isn't specific to either phase belongs, so it doesn't get lost or duplicated across both phase sections.

## Step 6 — Phase 1 functional draft (API)
...
Replace the `*(Not yet drafted.)*` placeholder under `## Phase 1: API` (written in Step 5) with this draft, structured as a checklist-style breakdown by layer (Domain / Application / Infrastructure / API / Contracts / Tests), mirroring the "What Changed" layer grouping used in `pull-request-prep`.
...
## Step 7 — Phase 1 code draft (API)

Once Step 6 is confirmed, expand each item in the Phase 1 section into actual code: real class/record signatures, method bodies where the logic isn't obvious boilerplate, route strings, validator rules, DI registration snippets, and factory method signatures. Use the patterns and templates in the `new-endpoint` command and `ddd-clean-architecture` skill as the baseline shape, adapted to this feature's actual types and rules — don't reproduce their placeholder syntax verbatim.

Replace the Step 6 functional bullets in `docs/plans/{feature-name}.md` under `## Phase 1: API` with this code-level detail (keep the layer-grouped structure, but each item is now a fenced code block instead of a one-line bullet).

Show the update in chat and ask: **"Does the Phase 1 code look right? Once you confirm, I'll move on to Phase 2 (UI)."**

Do not proceed until confirmed.

## Step 8 — Phase 2 functional draft (UI)

Same shape as Step 6, but for the Blazor side. The screens/actions/data needs were already sketched and confirmed in Step 4's flow — this step fleshes that flow out into concrete files and components, it doesn't re-litigate what the flow is:
...
Replace the `*(Not yet drafted.)*` placeholder under `## Phase 2: UI` (written in Step 5) with this draft (same layer/checklist style, adapted to Blazor: Pages / Components / API Client / Tests).
...
## Step 9 — Phase 2 UI mockups (HTML)

Once Step 8 is confirmed, and before writing any Blazor code, produce static HTML mockups of the new/changed pages for the user to review. Skip this step entirely (and say so) only if Step 8 concluded the feature has no real UI surface. These mockups give the flow confirmed in Step 4 its actual visual design — they should not change what screens/actions exist, only how they look.
...
For each new or substantially-changed page identified in Step 8, decide which kind of page it is:
...
- Use realistic sample data and the feature's actual field names/labels/categories (pulled from the Step 8 draft) — a mockup should read as this feature, not a generic template.
...
Do not proceed until confirmed. If the user asks for changes, update the mockup file(s) in place and re-confirm before moving to Step 10 — the same "don't silently fold unconfirmed changes into the next stage" rule that applies to every other gate applies here too.

## Step 10 — Phase 2 code draft (UI)

Once Step 9 is confirmed (or skipped, per Step 9's own condition), expand each item into actual `.razor`/`.razor.cs` code: markup, `@code` blocks, event handlers, DirtyFormGuard wiring if applicable, and the Refit client call. Use the confirmed mockup(s) as the source of truth for markup structure/classes/layout, adapted into Razor. Replace the Step 8 functional bullets under `## Phase 2: UI` with this code-level detail, same fenced-code-per-item structure as Step 7.
