...
- **`GameScore` carries `SquadId`/`BowlerId` directly**, not as a shadow FK reached through a parent navigation. `Squad`'s shadow FK to `Tournament` works because `Squad` is only ever reached by traversing `Tournament.Squads` — there's no equivalent parent here, since `ScoreCard` isn't an EF entity. `GameScore` has to be self-sufficient for a command handler to query it directly by `(SquadId, BowlerId)`, and both columns are real foreign keys to `squads`/`bowlers` — see Persistence.
- **No repository layer.** This codebase is Vertical Slice Architecture, not Clean Architecture, despite the `ddd-clean-architecture` skill's name — command handlers query `AppDbContext` directly (`appDbContext.Set<GameScore>()`), same as every other handler in `Features/Tournaments`. No `IScoreCardRepository`/`IGameScoreRepository` abstraction gets introduced for this feature.
- **Table name: `squad_scores`.** Not `QualifyingScores` (round-specific — would force a rename the day Cashers reuses this shape). Not `ScoreCards` (nothing persisted under that name). Not `game_scores` (redundant — `squad_` is what actually disambiguates this from a future match-play table, since match play scores are scoped by `MatchId`, not `SquadId`, and could never live in this table regardless of what it's called).
- **`GameScore.Value` must be 0–300.** Owned by `GameScore`'s own internal factory/update method — a structural, entity-owned invariant (Always-Valid Child Entity pattern), not merely a request-validator concern.
...
- **DNF is explicitly out of scope for this plan.** It's a fact about a bowler's Registration/Check-in for a squad, not about their scores — no schema or domain impact on `ScoreCard`/`GameScore`. Flagged in Deferred because it will matter later (standings sort order, gating a live squad-results view on "everyone who isn't DNF has finished this game"). Now designed in `registration.md`: DNF lives on `Registration`, gated on the entry fee being paid, and is independent of games bowled — a DNF may have zero `GameScore` rows behind it. No change to the decision here, just confirmation.
...
- **Legacy sync semantics: full replace per squad.** Matches nebamgmt-v3's own "delete all, recreate" behavior for that round. A bowler absent from the incoming payload has their `ScoreCard` removed entirely; a bowler present has their `GameScore` collection replaced wholesale. Risk flagged, not solved here: once Registration/DNF exists, a legacy sync could stomp a fact the website-only side set (legacy has no concept of DNF) — needs a real answer when that handler is built, not before.
- **Real-time editing is the workload that confirmed the aggregate grain.** Planned UI: a spreadsheet-like score entry screen where each field blur sends one `(SquadId, BowlerId, GameNumber, Score)` change over SignalR. That's one small `ScoreCard` loaded, one `GameScore` added/updated/removed, one save — cheap per keystroke. A coarser aggregate would mean every blur event loads and locks every bowler in the squad.
...
public readonly partial struct GameScoreId;
```

### `GameScore`

`src/Neba.Api/Features/Tournaments/Domain/GameScore.cs`:
...
{
    /// <summary>
    /// Gets the unique identifier for this game score.
    /// </summary>
    public required GameScoreId Id { get; init; }
    ...
    // EF-only navigations, needed for the real foreign keys configured in GameScoreConfiguration.
    ...
    internal static ErrorOr<GameScore> Create(SquadId squadId, BowlerId bowlerId, short gameNumber, int score)
    {
    ...
    private static ErrorOr<Success> ValidateScore(int score)
        => score is < 0 or > 300
            ? GameScoreErrors.InvalidValue(score)
            : Result.Success;
}
```

`src/Neba.Api/Features/Tournaments/Domain/GameScoreErrors.cs`:
...
internal static class GameScoreErrors
{
    public static Error InvalidValue(int score)
        => Error.Validation(
            code: "GameScore.Value.Invalid",
...
/// its identity is the (SquadId, BowlerId) pair; the only persisted rows are its <see cref="GameScore"/> children.
/// </summary>
    ...
    : AggregateRoot
{
    ...
    private readonly List<GameScore> _games = [];

    /// <summary>
    /// The recorded games for this bowler in this Squad.
    /// </summary>
    public IReadOnlyCollection<GameScore> Games => _games;
    ...
    public ErrorOr<Success> RecordGame(short gameNumber, int score, short expectedGameCount)
    {
        ...
        var created = GameScore.Create(SquadId, BowlerId, gameNumber, score);
        if (created.IsError)
        {
...
`GameScore.Create`/`UpdateValue` are `internal` per the Always-Valid Child Entity pattern — only `ScoreCard` constructs or mutates them. `ScoreCard.Create` is `public`: it's the aggregate root, not nested under anything else.

## Persistence

**Table**: `squad_scores`, mapping `GameScore` directly (there's no `ScoreCard` table to map). Both `SquadId` and `BowlerId` are real foreign keys, not just indexed columns — following the cross-aggregate-reference precedent `HighBlockAward.BowlerId`/`HallOfFameInduction.BowlerId` already use (real ULID value as the FK, `HasPrincipalKey` pointing at the target's ULID alternate key, since that's the value `GameScore` actually holds — not the target's internal shadow-int PK):
...
internal sealed class GameScoreConfiguration : IEntityTypeConfiguration<GameScore>
{
    public void Configure(EntityTypeBuilder<GameScore> builder)
    {
        ...
        builder.Property(gameScore => gameScore.Id)
            .IsUlid();

        builder.HasAlternateKey(gameScore => gameScore.Id);

        builder.Property(gameScore => gameScore.SquadId)
            .IsUlid(SquadConfiguration.ForeignKeyName)
            .IsRequired();

        builder.HasOne(gameScore => gameScore.Squad)
            .WithMany()
            .HasForeignKey(gameScore => gameScore.SquadId)
            .HasPrincipalKey(squad => squad.Id)
            .OnDelete(DeleteBehavior.Cascade);

        builder.Property(gameScore => gameScore.BowlerId)
            .IsUlid(BowlerConfiguration.ForeignKeyName)
            .IsRequired();

        builder.HasOne(gameScore => gameScore.Bowler)
            .WithMany()
            .HasForeignKey(gameScore => gameScore.BowlerId)
            .HasPrincipalKey(bowler => bowler.Id)
            .OnDelete(DeleteBehavior.Cascade);

        builder.Property(gameScore => gameScore.GameNumber)
            .HasColumnName("game_number")
            .IsRequired();

        builder.Property(gameScore => gameScore.Value)
            .HasColumnName("value")
            .IsRequired();

        builder.HasAlternateKey(gameScore => new { gameScore.SquadId, gameScore.BowlerId, gameScore.GameNumber });
    }
}
```
...
**No repository — `AppDbContext` gets a `GameScores` `DbSet<GameScore>`** (alongside `Sponsors`, `Bowlers`, etc.), queried directly from the command handler:

```csharp
var scoreCardRows = await appDbContext.GameScores
    .Where(gameScore => gameScore.SquadId == command.SquadId && gameScore.BowlerId == command.BowlerId)
    .ToListAsync(cancellationToken);
```

with the handler reconstructing/mutating a `ScoreCard` in memory from those rows, then adding/updating/removing the underlying `GameScore` tracked entities before `SaveChangesAsync` — no `IScoreCardRepository`/`IGameScoreRepository` abstraction. Accessibility of the `DbSet` property (`public` vs. `internal`) follows whichever of the existing precedents (`Sponsors` vs. `HistoricalTournamentChampions`) fits once the consuming handlers are actually written.
...
- Read side: a query projecting `squad_scores` rows directly into whatever DTOs a results/standings page needs — no involvement from `ScoreCard`/`GameScore` at all, same as every other query in this codebase bypasses the write-side aggregate.
...
**Definition**: One bowler's game-by-game scores for a single Squad. A Score Card exists for exactly one (Squad, Bowler) pair. It is not itself a persisted record — its identity is that pair, and it's a grouping over that bowler's Game Scores for the Squad, loaded and saved as a unit.

**Rules**:

- A Score Card holds at most one Game Score per game number
- A game number may not exceed the Tournament format's expected game count for that Squad
- Game Scores are added, corrected, and removed exclusively through the Score Card

**In Code**:

- Namespace: `Neba.Api.Features.Tournaments.Domain`
- Type: `ScoreCard` (aggregate root; not itself persisted)
- Property: `ScoreCard.Games` (`IReadOnlyCollection<GameScore>`)
...
- Namespace: `Neba.Api.Features.Tournaments.Domain`
- Type: `GameScore` (child entity of `ScoreCard`)
- Identity type: `GameScoreId` (ULID-backed strongly-typed ID)
...
- **DNF** — belongs to `Registration` (see `registration.md`), not to `ScoreCard`. Confirmed there: DNF requires the entry fee to be paid, and is independent of games bowled — a DNF may have zero `GameScore` rows behind it (the rare case: a withdrawal request the TD declines before the bowler ever throws a ball). Will eventually interact with scoring at the read side only: sorting DNF bowlers to the bottom of standings, and gating a live squad-results view (SSE) on "every non-DNF bowler has finished this game" before advancing the displayed game number. No schema or domain impact on this plan.
- **Forfeit** — belongs to the future `Entry` aggregate (see `registration.md`'s Deferred section), not to `ScoreCard`. Same read-side shape as DNF above: when an Entry forfeits (e.g., a doubles pair changes partners between squads), the raw `GameScore` rows already bowled stay recorded — matching this plan's "raw scores are never deleted" rule — and are simply excluded from standings/advancement at query time. When the future standings/live-results query is designed, it needs to exclude on *both* signals (DNF and Forfeit), not just DNF.
- **Registration itself** — which bowlers are entered in a squad at all. `ScoreCard`/`GameScore` don't check that a `BowlerId` is actually registered for the `SquadId` it's scoring against; that's `Registration`'s job (see `registration.md`, itself deferred to its own future implementation pass), not scoring's. `Registration`'s grain — `(SquadId, BowlerId)` — matches `ScoreCard`'s exactly, though they remain separate aggregates with separate tables.
- **The 2026 data import** — `data-migration/qualifying-scores/*.json` is confirmed to already be the right shape for `GameScore`, but the actual import tooling/job isn't designed here.
