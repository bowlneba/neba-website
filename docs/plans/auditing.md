...
| Failure isolation | `ResilientAuditDataProvider` wraps the configured data provider so a Table Storage outage degrades to a logged warning instead of failing the caller; Discord-notified fire-and-forget so the notification itself never adds latency to the audited operation |
...
- `AddAuditing()` / `AuditingConfiguration` — `src/Neba.Api/Auditing/AuditingConfiguration.cs`
  - `ForContext<AppDbContext>` opt-in list (grows as new aggregates need audit coverage — check the source for the current list): `Bowler`, `Season`, `Tournament`, `OilPattern`, `HallOfFameInduction`, `HighAverageAward`, `HighBlockAward`, `BowlerOfTheYearAward`, `BowlingCenter`, `Sponsor`, `Article`, `PendingUpload`, `Squad`, `SquadScore`, `TournamentResult`
...
- `ResilientAuditDataProvider` — swallow-and-log wrapper around the resolved data provider, also posts a fire-and-forget Discord alert on failure (`src/Neba.Api/Auditing/ResilientAuditDataProvider.cs`)
...
### Phase 4 — Docs, hardening, cleanup (done)

- `docs/architecture/backend.md`'s "Application Auditing" section documents the guidelines, audit sources, and configuration entry point
- Failure isolation (`ResilientAuditDataProvider`) implemented and tested, including the fire-and-forget Discord alert on failure
- Security/identity audit trail isolated to its own table
- Full regression pass across unit + integration suites

Still outstanding (infra/RBAC, outside app code — track as a deployment checklist item):
- Confirm managed identity / RBAC for the audit tables is write-only (no delete) in production
