...
- **UI entry point**: no separate admin-gated list page. The existing public `/sponsors` page (`Sponsors.razor`) is the entry point for both audiences — it already calls the permission-aware `ISponsorsApi.ListActiveSponsorsAsync` (Phase 1's amendment: an authenticated caller with a sponsor-management permission gets every sponsor, active and inactive; everyone else gets active-only) and already has a `<AuthorizeView Policy="@Permissions.CreateSponsor.PolicyName">`-gated `FabCreateButton Href="/sponsors/new" Label="Create Sponsor"`. There is no reason to stand up a second route that re-fetches the same gated data — see Phase 2 for what (if anything) needs to change on the existing page to visually surface inactive sponsors to a permitted caller.
    ...
    : AggregateRoot
{
    ...
    /// <summary>
    /// Creates a new sponsor. <paramref name="id"/> is caller-supplied (not generated internally) —
    /// production callers pass <c>SponsorId.New()</c>; test factories pass a deterministic ID for
    /// stable Verify snapshots. If <paramref name="slug"/> is null or empty, the slug is generated
    ...
    : ITitleSponsorPolicy
{
    ...
    {
        var titleSponsorExists = await appDbContext.Sponsors
            .AnyAsync(s => s.Tier == SponsorTier.TitleSponsor, cancellationToken);

        return !titleSponsorExists;
    }
}
```

Register in `InfrastructureConfiguration.cs` (or wherever Sponsors' feature-scoped services are added) as scoped: `services.AddScoped<ITitleSponsorPolicy, TitleSponsorPolicy>();`. This is a narrow, single-purpose repository-shaped dependency — it does not generalize into a full `ISponsorRepository`; every other Sponsor query/command in this plan still goes through `AppDbContext` directly, matching the rest of the codebase's convention. The exception here is deliberate: it's what keeps `Sponsor.Create` testable with a bare `bool`.
    ...
    : ICommandHandler<CreateSponsorCommand, CreatedSponsor>
{
    public async Task<ErrorOr<CreatedSponsor>> HandleAsync(CreateSponsorCommand command, CancellationToken cancellationToken)
    {
        ...
        // Fail-safe default: only queried (and only ever true) when the command is itself trying to
        // create a Title-tier sponsor — every other tier passes Sponsor.Create's default of `false`,
        // which is irrelevant to the guard clause since it only checks when tier == TitleSponsor.
        var isTitleSponsorshipAvailable = command.Tier == SponsorTier.TitleSponsor
            && await titleSponsorPolicy.IsTitleSponsorSlotAvailableAsync(cancellationToken);
...
- **`Sponsors.razor`** (`@page "/sponsors"`, existing public page) — already implements the entry point described above (`ListActiveSponsorsAsync` call + gated `FabCreateButton`); no new route needed. What it does **not** yet do is visually distinguish inactive sponsors for a permitted caller — today `TitleSponsor`/`PremierSponsors`/`AssociationSponsors` group purely by `Tier`, so an inactive sponsor returned to a management-permitted caller renders identically to an active one, indistinguishable in the tier sections (a former Title Sponsor that's now inactive would even render in the single "top slot" as if it still holds the tier). **Flag for confirmation before implementing**: does this page need an **Inactive Sponsors** section (plain list, no tier styling, visible only when the caller has the management permission and at least one inactive sponsor exists) added below the existing tier sections, mirroring what a separate manage page would have shown? Computed client-side: `InactiveSponsors => _sponsors.Where(s => !s.IsCurrentSponsor)`, and the existing tier groupings (`TitleSponsor`, `PremierSponsors`, `AssociationSponsors`) would need `.Where(s => s.IsCurrentSponsor)` added so an inactive sponsor never leaks into a tier section.
