...
Per `docs/api/software-backdoor-plan.md`'s Testing section, all five layers apply — and, since a temporary backdoor action is only ever deleted as a unit, **all of them live in one file**, `tests/Neba.Api.Tests/Legacy/Bowlers/NewBowlerTests.cs` (multiple `[Fact]`/`[Theory]`-bearing test classes, one per production type, in the single file), mirroring `Legacy/Bowlers/NewBowler.cs`'s own single-file shape. This is a deliberate deviation from the codebase's usual one-class-per-file test layout — it means the entire test suite for this action is removed in the same delete as the source at sunset, with no leftover test files to hunt down across the tree.

1. **Validator unit test** — `NewBowlerRequestValidatorTests`, standard FluentValidation shape.
2. **Endpoint integration test** — `NewBowlerEndpointTests`, a real `WebApplication` on `Microsoft.AspNetCore.TestHost` (not `WebApplicationFactory<Program>` — this endpoint is a standalone minimal-API delegate with no dependency on `Program`), asserts `400`/`202` + the right job (`NewBowlerSyncJob.SyncAsync`, right `bowlerId`) enqueued via a `MockBehavior.Strict` `IBackgroundJobClient`. `LegacyApiKeyFilter`'s `401`/auth behavior is already covered independently by `LegacyApiKeyFilterTests`, so it isn't re-asserted here.
3. **`Bowler.CreateFromLegacy` unit tests** — `LegacyBowlerExtensionsTests`, valid-mapping and validation-failure cases.
4. **Sync job + legacy query tests** — `NewBowlerSyncJobTests`, against a real `AppDbContext` (`AppDbContextFixture`, Testcontainers Postgres — already standard infra in this project) for the write side, and a real ADO.NET connection for the legacy read side. **The legacy connection is a `NpgsqlConnection` against the same Postgres container, not `Testcontainers.MsSql`** — a `CREATE TEMP TABLE Bowlers (...)` shaped like the real legacy schema, scoped to that one connection so it needs no cleanup and can't collide with the fixture's own schema. This was chosen over adding the `Testcontainers.MsSql` package originally sketched below, and over an in-memory SQLite connection (tried first): SQLite's dynamic typing returns `Int64`/`string` for every `INTEGER`/`TEXT` column regardless of the declared type, and Dapper's record-constructor mapping for `LegacyBowlerRow` requires the reader's column types to exactly match its constructor parameters (`int`, `DateTime?`, ...) — Postgres's strict typing (`int4 → Int32`, `timestamp → DateTime`) satisfies that the same way `Microsoft.Data.SqlClient` would against the real MSSQL schema, without a second container type. Covers: not-found, gender mapping (`0`/`1`/other → `Male`/`Female`/`null`), date-of-birth mapping, suffix mapping (recognized/blank/unrecognized), and the create-failure (invalid mapped name) path.
5. **Idempotency test** — same `NewBowlerSyncJobTests` class: run `SyncAsync` twice for the same legacy id, assert the second run doesn't create a duplicate `Bowler` row and leaves the first run's row untouched (per the strictly-create-only decision above, not an upsert).

**Two bugs surfaced and fixed while reviewing the implemented `NewBowler.cs` against this plan** (the file did not compile before these fixes):

- `LogLegacyBowlerCreateFailed(legacyBowlerId, bowler.Errors)` passed the raw `List<Error>` where the `[LoggerMessage]` method expects a `string` — fixed to `string.Join("; ", bowler.Errors.Select(e => e.Description))`, matching the sketch below.
- Dapper's `DAP005` analyzer (`TreatWarningsAsErrors` in this repo) flagged the `QuerySingleOrDefaultAsync` call, since this is the codebase's first Dapper usage and Dapper.AOT isn't enabled project-wide. Suppressed locally with `#pragma warning disable/restore DAP005` around the call rather than adding the AOT package for one query.
