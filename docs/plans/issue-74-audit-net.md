...
- No table-name allowlist needed: `.UseOptIn().Include<T>()` on the `ForContext<SecurityDbContext>` configuration already restricts audit events to `ApplicationUser`/`IdentityUserRole<Ulid>` — same mechanism as `ForContext<AppDbContext>` (see the corrected 1e/1f note above; `AuditedTableNames`/`IsAuditedTable` were dropped from the plan and were never implemented).
