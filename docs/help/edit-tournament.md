...
3. On the **Edit Tournament** page, the form is pre-filled with the tournament's current values. Update any of:
   ...
   - **Logo** — if a logo is already set, it's shown with a **Remove current logo** button. Drag and drop, or click to browse for, a replacement image (up to 5 MB). If the tournament has no logo of its own (nothing was ever set, or you remove the current one), it displays its title sponsor's logo instead, as long as one is assigned to the tournament and has a logo of its own (see [`docs/help/manage-tournament-sponsors.md`](manage-tournament-sponsors.md)); if neither is set, a default logo based on the tournament type is shown.
