...
That set now includes `News.CreateArticle`, `News.EditArticle`, and `News.DeleteArticle` — adding a permission to this list is the only change needed to extend it further; no additional policy-wiring changes are required.
...
- `Roles.Webmaster` — has `Permissions.CreateArticle` and `Permissions.DeleteArticle` directly (but **not** `Permissions.EditArticle` — see `src/Neba.Api/Security/Infrastructure/SecurityRoleSeeder.cs`).
- `Roles.Admin` — has every permission via `Permissions.List`, including `CreateArticle`, `EditArticle`, and `DeleteArticle`.
...
- `EditArticleEndpoint` — `.Policies(Permissions.EditArticle.PolicyName)`; matched by `<AuthorizeView Policy="@Permissions.EditArticle.PolicyName">` gating the entire `EditArticle.razor` page (`/news/{slug}/edit`) and the edit buttons in `ArticleCard.razor`/`NewsList.razor`.
