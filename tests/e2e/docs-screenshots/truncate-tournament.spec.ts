...
/**
 * Generates the screenshots embedded in docs/help/truncate-tournament.md.
 *
 ...
 *
 * The truncate action here is cancelled rather than confirmed, so this script can be
 * re-run against the same mock data without needing a reset step, and so it doesn't
...
const outDir = path.join('docs', 'help', 'images', 'truncate-tournament');
...
test.describe('truncate-tournament help screenshots', () => {
  ...
  test('detail truncate button + confirm dialog', async ({ page }) => {
    await page.goto(`/tournaments/${MOCK_TOURNAMENT_STATUS_ACTIONS_ID}`);
    await page.waitForSelector('h1');

    await expect(page.getByRole('button', { name: 'Mark Truncated' })).toBeVisible();

    await page.screenshot({ path: path.join(outDir, 'detail-truncate-button.png') });

    await page.getByRole('button', { name: 'Mark Truncated' }).click();
