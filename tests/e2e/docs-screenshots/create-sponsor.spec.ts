...
/**
 * Generates the screenshots embedded in docs/help/create-sponsor.md.
 *
 ...
 * script stops at the blank, pre-submit form rather than actually creating a mock sponsor.
 */

const outDir = path.join('docs', 'help', 'images', 'create-sponsor');

test.describe.configure({ mode: 'serial' });

test.describe('create-sponsor help screenshots', () => {
  ...
  test('sponsors list FAB + blank create form', async ({ page }) => {
    await page.goto('/sponsors');
    await page.waitForSelector('h1');

    await expect(page.getByRole('link', { name: 'Create Sponsor' })).toBeVisible();

    await page.screenshot({ path: path.join(outDir, 'sponsors-list-fab.png') });
