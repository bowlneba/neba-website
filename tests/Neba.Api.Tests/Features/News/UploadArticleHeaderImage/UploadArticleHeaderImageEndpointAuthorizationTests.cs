...
public sealed class UploadArticleHeaderImageEndpointAuthorizationTests(SecurityDbContextFixture fixture)
    : IClassFixture<SecurityDbContextFixture>, IAsyncLifetime
{
    ...
    public async ValueTask InitializeAsync()
    {
        ...
        builder.Services
            ...
            {
                options.Assemblies = [typeof(UploadArticleHeaderImageEndpoint).Assembly];
                options.Filter = type => type == typeof(UploadArticleHeaderImageEndpoint);
            })
            .AddVersioning();
    ...
    private async Task<HttpResponseMessage> SendUploadAsync(string? accessToken, CancellationToken ct)
    {
        ...
        content.Add(fileContent, "File", "header.png");

        using var request = new HttpRequestMessage(HttpMethod.Post, "/news/header-image")
        {
            Content = content
        };
    ...
    [Fact(DisplayName = "POST /news/header-image returns 401 when no access token is provided")]
    ...
    [Fact(DisplayName = "POST /news/header-image returns 403 when the token lacks the News.CreateArticle permission")]
    ...
    [Fact(DisplayName = "POST /news/header-image returns 403 when the token has roles but no permission claims")]
    ...
    [Fact(DisplayName = "POST /news/header-image returns 200 when the token has the News.CreateArticle permission")]
