...
public sealed class UploadSponsorLogoEndpointAuthorizationTests(SecurityDbContextFixture fixture)
    : IClassFixture<SecurityDbContextFixture>, IAsyncLifetime
{
    ...
    public async ValueTask InitializeAsync()
    {
        ...
        builder.Services
            ...
            {
                options.Assemblies = [typeof(UploadSponsorLogoEndpoint).Assembly];
                options.Filter = type => type == typeof(UploadSponsorLogoEndpoint);
            })
            .AddVersioning();
    ...
    private async Task<HttpResponseMessage> SendUploadAsync(string? accessToken, CancellationToken ct)
    {
        ...
        using var request = new HttpRequestMessage(HttpMethod.Post, "/sponsors/logo")
        {
            Content = content
        };
    ...
    [Fact(DisplayName = "POST /sponsors/logo returns 401 when no access token is provided")]
    ...
    [Fact(DisplayName = "POST /sponsors/logo returns 403 when the token lacks the Sponsors.CreateSponsor permission")]
    public async Task Upload_ShouldReturn403_WhenTokenLacksCreateSponsorPermission()
    {
        // Arrange
        var ct = TestContext.Current.CancellationToken;
    ...
    [Fact(DisplayName = "POST /sponsors/logo returns 403 when the token has roles but no permission claims")]
    ...
    [Fact(DisplayName = "POST /sponsors/logo returns 200 when the token has the Sponsors.CreateSponsor permission")]
    public async Task Upload_ShouldReturn200_WhenTokenHasCreateSponsorPermission()
    {
        // Arrange
        var ct = TestContext.Current.CancellationToken;
