...
public sealed class TruncateTournamentRequestValidatorTests
{
    private const string ValidId = "01000000000000000000000001";

    private readonly TruncateTournamentRequestValidator _validator = new();

    [Fact(DisplayName = "Validate should succeed when the request is valid")]
    public void Validate_ShouldSucceed_WhenRequestIsValid()
    {
        var result = _validator.Validate(new TruncateTournamentRequest { Id = ValidId });
    ...
    {
#nullable disable
        var result = _validator.Validate(new TruncateTournamentRequest { Id = null });
#nullable enable

        result.IsValid.ShouldBeFalse();
        result.Errors.ShouldContain(e =>
            e.PropertyName == nameof(TruncateTournamentRequest.Id)
            && e.ErrorCode == "TruncateTournamentRequest.IdRequired");
    }
    ...
    {
        var result = _validator.Validate(new TruncateTournamentRequest { Id = "SHORT" });

        result.IsValid.ShouldBeFalse();
        result.Errors.ShouldContain(e =>
            e.PropertyName == nameof(TruncateTournamentRequest.Id)
            && e.ErrorCode == "TruncateTournamentRequest.IdInvalidLength");
    }
}