...
public sealed class UploadTournamentLogoEndpointAuthorizationTests(SecurityDbContextFixture fixture)
    : IClassFixture<SecurityDbContextFixture>, IAsyncLifetime
{
    ...
    public async ValueTask InitializeAsync()
    {
        ...
        fileStorageServiceMock
            ...
            .Returns(new Uri("https://storage.example.com/test-container/logo.png"));
        builder.Services.AddSingleton(fileStorageServiceMock.Object);

        builder.Services
            .AddFastEndpoints(options =>
            {
                options.Assemblies = [typeof(UploadTournamentLogoEndpoint).Assembly];
                options.Filter = type => type == typeof(UploadTournamentLogoEndpoint);
            })
            .AddVersioning();
    ...
    private async Task<HttpResponseMessage> SendUploadAsync(string? accessToken, CancellationToken ct)
    {
        ...
        using var request = new HttpRequestMessage(HttpMethod.Post, "/tournaments/logo")
        {
            Content = content
        };
    ...
    [Fact(DisplayName = "POST /tournaments/logo returns 401 when no access token is provided")]
    ...
    [Fact(DisplayName = "POST /tournaments/logo returns 403 when the token lacks the Tournaments.CreateTournament permission")]
    public async Task Upload_ShouldReturn403_WhenTokenLacksCreateTournamentPermission()
    {
        // Arrange
        var ct = TestContext.Current.CancellationToken;
    ...
    [Fact(DisplayName = "POST /tournaments/logo returns 403 when the token has roles but no permission claims")]
    ...
    [Fact(DisplayName = "POST /tournaments/logo returns 200 when the token has the Tournaments.CreateTournament permission")]
    public async Task Upload_ShouldReturn200_WhenTokenHasCreateTournamentPermission()
    {
        // Arrange
        var ct = TestContext.Current.CancellationToken;
