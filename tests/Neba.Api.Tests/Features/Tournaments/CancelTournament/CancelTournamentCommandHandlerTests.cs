...
public sealed class CancelTournamentCommandHandlerTests(AppDbContextFixture fixture)
    : IClassFixture<AppDbContextFixture>, IAsyncLifetime
{
    ...
    private CancelTournamentCommandHandler CreateHandler()
    ...
    public async Task HandleAsync_ShouldReturnNotFound_WhenTournamentDoesNotExist()
    {
        ...
        var command = new CancelTournamentCommand { TournamentId = TournamentId.New() };
    ...
    [Fact(DisplayName = "HandleAsync returns success and marks the tournament cancelled when it is scheduled")]
    public async Task HandleAsync_ShouldReturnSuccessAndMarkCancelled_WhenTournamentIsScheduled()
    {
        // Arrange
        var ct = TestContext.Current.CancellationToken;
        var tournament = await SeedTournamentAsync(ct);
        var handler = CreateHandler();
        var command = new CancelTournamentCommand { TournamentId = tournament.Id };
        ...
        reloaded.Status.ShouldBe(TournamentStatus.Cancelled);
    }
    ...
    public async Task HandleAsync_ShouldReturnAlreadyFinalized_WhenTournamentIsAlreadyFinalized()
    {
        // Arrange
        var ct = TestContext.Current.CancellationToken;
        var tournament = await SeedTournamentAsync(ct, status: TournamentStatus.Truncated);
        var handler = CreateHandler();
        var command = new CancelTournamentCommand { TournamentId = tournament.Id };
    ...
    [Fact(DisplayName = "HandleAsync invalidates the tournament and season cache tags when the tournament is cancelled")]
    public async Task HandleAsync_ShouldInvalidateTournamentAndSeasonCacheTags_WhenTournamentIsCancelled()
    {
        // Arrange
        var ct = TestContext.Current.CancellationToken;
        var tournament = await SeedTournamentAsync(ct);
        ...
        var tournamentCacheAfterCancel = await cache.GetOrSetAsync(
            tournamentCacheKey,
            _ => Task.FromResult("fresh-tournament"),
            token: ct);
        tournamentCacheAfterCancel.ShouldBe("fresh-tournament");

        var seasonCacheAfterCancel = await cache.GetOrSetAsync(
            seasonCacheKey,
            _ => Task.FromResult("fresh-season-tournaments"),
            token: ct);
        seasonCacheAfterCancel.ShouldBe("fresh-season-tournaments");
    }
    ...
    public async Task HandleAsync_ShouldNotInvalidateCache_WhenTournamentIsAlreadyFinalized()
    {
        ...
        var tournamentCacheAfterCancel = await cache.GetOrSetAsync(
            ...
            _ => Task.FromResult("fresh-tournament"),
            token: ct);
        tournamentCacheAfterCancel.ShouldBe("cached-tournament");
    }
}