...
internal sealed class CancelTournamentEndpoint(Messaging.ICommandHandler<CancelTournamentCommand, Success> commandHandler)
    : Endpoint<CancelTournamentRequest>
{
    private readonly Messaging.ICommandHandler<CancelTournamentCommand, Success> _commandHandler = commandHandler;
    ...
    public override async Task HandleAsync(CancelTournamentRequest req, CancellationToken ct)
    {
        var command = new CancelTournamentCommand { TournamentId = new TournamentId(req.Id) };
