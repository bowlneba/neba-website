...
2. **Hangfire `GlobalJobFilters.Filters` + `[AuditJobExecutionFilter]` double-application** — `HangfireGlobalAuditFilterIntegrationTests` (`tests/Neba.Api.Tests/BackgroundJobs/`) called `.AddAuditJobExecutionFilter(...)` on `IGlobalConfiguration` in production code (`BackgroundJobConfiguration.cs`), which resolves to Hangfire's `UseFilter<T>()` — this registers into the static, process-wide `GlobalJobFilters.Filters` collection, applying to **every** job in the process, including ones that already carry their own method-level `[AuditJobExecutionFilter]` attribute (e.g. `AuditJobExecutionIntegrationTests`'s `AuditableTestJob`, in an entirely different test class/collection). When both the global registration and a method-level attribute apply to the same job execution, Hangfire runs both filter instances — and `AuditJobExecutionFilterAttribute.OnPerforming`/`OnPerformed` (`Audit.Hangfire`) key their `IAuditScope` into `PerformContext.Items` under **fixed strings**, not per-instance keys, so the second `OnPerforming` clobbers the first's entry and one of the two `OnPerformed` calls resolves the *wrong* scope. Root-caused via decompiling `Audit.Hangfire`/`Hangfire.Core` with `ilspycmd`: `HangfireJobExecutionEvent.IsSuccess => Exception == null`, and `Exception` is only ever set inside `OnPerformed` from `context.Exception` — Hangfire propagates **any** filter's thrown exception into `context.Exception` for the rest of the chain, so if the *other* filter instance's own audit write throws (e.g. writing to a table/connection that isn't actually relevant to this job), the legitimate filter's `OnPerformed` reads that borrowed exception and marks an otherwise-successful job's audit event `IsSuccess: false`. **This is not just test-state leakage — it's a real double-filter-application hazard in the production Hangfire config itself**, latent for any job that both opts in via attribute and is covered by the global registration. **Fix — solved at the root, in production code**: added an `.AuditWhen(...)` predicate to the global filter registration in `BackgroundJobConfiguration.cs` that skips any job whose method or declaring type already carries `[AuditJobExecutionFilter]`, so the two instances can never simultaneously apply to one job — this holds regardless of test ordering, collection sequencing, or cleanup timing, unlike relying on `GlobalJobFilters.Filters.Remove<T>()` in a test's `DisposeAsync()` (which was tried first, then hardened with `try`/`finally`, and *still* didn't reliably resolve the CI-only flake, because it depends on assumptions about cross-class test scheduling that couldn't be locally verified or reproduced despite 8+ repeated local runs).
   - **Lesson**: when two independently-registered filters/handlers/interceptors of the same library type can apply to the same unit of work (a job, a request, an event), check whether the library scopes its own per-execution state (context items, ambient scope, etc.) by *instance* or by a *fixed key* — a fixed key means at most one such filter can safely be active per execution, and if your code (or your tests) can register more than one, that's a bug to fix by mutual exclusion (an `AuditWhen`/predicate/guard), not by hoping registration order or test isolation prevents overlap.

**When adding a new integration test that spins up a real host/library configuration (`WebApplication`, `IGlobalConfiguration`, `IHostBuilder`, etc.) for the first time in the test suite**: check whether the library being configured exposes any `static`/global mutable state (options classes reached via a static singleton, `GlobalJobFilters`, `ValidatorOptions.Global`, `Audit.Core.Configuration`, Hangfire's static `LogProvider`, etc.). If so, either avoid touching the option that mutates it, or explicitly reverse/disable the mutation in `DisposeAsync()` **inside a `try`/`finally`** so the reversal isn't skipped if an earlier teardown step throws. Don't assume `[Collection("...Sequential")]` grouping is sufficient protection — it only serializes tests *within* the same collection; it does nothing to stop the static state leaking into tests in *other* collections/classes, which is where these bugs actually surfaced.
