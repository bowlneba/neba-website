...
2. **Hangfire `GlobalJobFilters.Filters` + `[AuditJobExecutionFilter]` double-application, plus a `WaitForEventAsync` ordering bug — a persistent, still only-partially-explained CI-only flake.** `AuditJobExecutionIntegrationTests.Execute_ShouldProduceAuditEvent_WhenJobSucceeds` intermittently failed in CI (never locally reproduced despite 8+ repeated full-suite and targeted-collection runs) with `auditEvent.JobExecution.IsSuccess` `False` for a job whose body provably succeeded. Three fixes were applied in sequence, each addressing a real, verified issue, but **CI still failed after the first two** — this entry documents all three plus the remaining open question so the next person doesn't have to re-derive them:
   ...
   - **Status**: (a)-(d) are all now fixed together — full `GlobalJobFilters.Filters.Clear()`, the `AuditWhen` guard (defends against a related but distinct hazard even though not the root cause here), `WaitForEventAsync`'s `MaxBy`/`InvalidOperationException` hardening (still correct/useful — retries were real, just for a different underlying reason), and now the `JobActivator.Current` reset, which is what actually stops job execution from failing before the body runs. `Execute_ShouldProduceAuditEvent_WhenJobSucceeds` still carries the diagnostic dump (all matching completed events' `Exception`/`EndDate`) in its `ShouldBeTrue` failure message — that dump is what turned this from a guessing game into a confirmed root cause, so keep it if any sibling test in this area starts flaking similarly.
