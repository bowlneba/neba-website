...
2. **Hangfire `GlobalJobFilters.Filters`** — `HangfireGlobalAuditFilterIntegrationTests` (`tests/Neba.Api.Tests/BackgroundJobs/`) called `.AddAuditJobExecutionFilter(...)` on `IGlobalConfiguration`, which resolves to Hangfire's `UseFilter<T>()` — this always registers into the static, process-wide `GlobalJobFilters.Filters` collection, never anything scoped to that test's own `BackgroundJobServer`/storage. Left registered, the filter (configured to write to that test's Azurite-backed table) kept firing for every job on every Hangfire server in the process afterward — including `AuditJobExecutionIntegrationTests`'s own separate in-memory server, whose job (`AuditableTestJob`) carries its own method-level `[AuditJobExecutionFilter]` attribute. **Exact failure mechanism**: `AuditJobExecutionFilterAttribute.OnPerforming`/`OnPerformed` (in `Audit.Hangfire`) stash the created `IAuditScope` in `PerformContext.Items` under **fixed string keys** (`"__AuditJobExecutionFilter:AuditScope"`/`AuditEvent`), not per-instance keys. When two instances of that same attribute type are active for one job (the method attribute + the leftover global one), the second `OnPerforming` overwrites the first's `Items` entry; then whichever `OnPerformed` runs first removes the key and finalizes its own scope, and the *other* filter's `OnPerformed` finds the key already gone and returns early — silently abandoning its own `IAuditScope` at its "start" (pre-completion) snapshot, so `IsSuccess` reads `false` forever even though the job succeeded. This is a real limitation in `Audit.Hangfire` itself (fixed string keys, not scoped to the filter instance), not just "our test leaked global state" — it would reproduce in production too if a job ever had both a method-level `[AuditJobExecutionFilter]` attribute and matched a globally-registered one. **Fix (two parts)**: (a) `GlobalJobFilters.Filters.Remove<AuditJobExecutionFilterAttribute>();` in `DisposeAsync()`; (b) put that removal in a `finally` block that runs *before* (or independent of) `IHostedService.StopAsync()`/`ServiceProvider.DisposeAsync()` — if either of those throws under load, execution never reaches cleanup code placed after them, silently leaving the global filter registered for the rest of the run. Critical cross-test cleanup must not be skippable by an earlier teardown step failing.
