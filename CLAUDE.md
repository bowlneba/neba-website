...
**This fix will not survive `dotnet format`/SonarQube unattended** — `dotnet_diagnostic.IDE0305.severity = warning` in `.editorconfig` ("Simplify collection initialization") actively suggests turning `new List<T>()` back into `[]`, and the pre-push Husky hook runs `dotnet format` and auto-commits any changes it makes. Every site fixed above therefore needs **both** of the following, not just one:
