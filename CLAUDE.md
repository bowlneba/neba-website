...
2. **Hangfire `GlobalJobFilters.Filters` + `[AuditJobExecutionFilter]` double-application, plus a `WaitForEventAsync` ordering bug — a persistent, still only-partially-explained CI-only flake.** `AuditJobExecutionIntegrationTests.Execute_ShouldProduceAuditEvent_WhenJobSucceeds` intermittently failed in CI (never locally reproduced despite 8+ repeated full-suite and targeted-collection runs) with `auditEvent.JobExecution.IsSuccess` `False` for a job whose body provably succeeded. Three fixes were applied in sequence, each addressing a real, verified issue, but **CI still failed after the first two** — this entry documents all three plus the remaining open question so the next person doesn't have to re-derive them:
   - **(a) `GlobalJobFilters.Filters` leak** — `HangfireGlobalAuditFilterIntegrationTests` calls `.AddAuditJobExecutionFilter(...)` on `IGlobalConfiguration` (in production code, `BackgroundJobConfiguration.cs`), which resolves to Hangfire's `UseFilter<T>()` and registers into the static, process-wide `GlobalJobFilters.Filters` — applying to **every** job in the process, including `AuditJobExecutionIntegrationTests`'s `AuditableTestJob` in a different test class. **Fix**: `GlobalJobFilters.Filters.Remove<AuditJobExecutionFilterAttribute>();` in that test's `DisposeAsync()`, wrapped in `try`/`finally` so it can't be skipped if `StopAsync()`/`DisposeAsync()` above it throws. Verified correct and necessary, but **did not resolve the CI flake alone**.
   ...
   - **(c) `WaitForEventAsync` picks the wrong event on retry** — the polling helper used `.FirstOrDefault(e => e.EventType == eventType && e.EndDate.HasValue)`. Hangfire has automatic retry behavior; if a job's first execution attempt fails for *any* reason (transient storage contention, filter interference, etc.), the retried (successful) attempt produces a **second**, separate completed audit event with the same `EventType` — and `FirstOrDefault` returns the stale first (failed) attempt by insertion order, not the actual final outcome. **Fix**: changed to `.Where(...).MaxBy(e => e.EndDate)` so the poll always resolves to the most recently completed event for that type, regardless of how many attempts occurred. This is defensive/robust regardless of what causes a first attempt to fail, and doesn't require knowing the exact trigger.
   - **Open question**: (a) and (b) are confirmed-real bugs worth having fixed regardless, and (c) makes the test robust to retries no matter the cause — but *why* `AuditableTestJob.Succeed`'s first execution attempt would transiently fail at all under CI (and never locally) is still not conclusively identified. If this resurfaces after (c), the next step is to add temporary diagnostic logging of `Provider.GetAllEvents()`'s full contents (not just the first match) on assertion failure, to see directly whether multiple events for the same `EventType` exist and what their `Exception` fields say.
   - **Lesson**: when two independently-registered filters/handlers/interceptors of the same library type can apply to the same unit of work (a job, a request, an event), check whether the library scopes its own per-execution state (context items, ambient scope, etc.) by *instance* or by a *fixed key* — a fixed key means at most one such filter can safely be active per execution. Separately: any test that polls for "the" event/record produced by an operation should assume the operation might produce more than one (retries, at-least-once delivery, etc.) and select the most recent/final one rather than the first match.
