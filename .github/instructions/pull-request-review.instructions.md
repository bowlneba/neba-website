...
- **Roles** — `CreateUserInput.Roles`/`CreateUserCommand.Roles` use plain role-name strings (`"Webmaster"`, `"Manager"`, ...), validated against the fixed `Roles.All` list. This was originally planned as a `RoleId` reference (roles are ASP.NET Identity rows, `ApplicationRole : IdentityRole<Ulid>`, not a SmartEnum) but the built flow found `Roles.cs` is six hardcoded constants seeded at startup with no admin UI to create/rename roles — roles function as a closed, compile-time vocabulary in practice, same shape as a SmartEnum's name, so a name string is the deliberate choice here, not a shortcut. Revisit if roles ever become dynamically DB-managed (an admin UI to create/rename roles) — at that point a name string stops being stable and `RoleId` becomes the right shape again.
